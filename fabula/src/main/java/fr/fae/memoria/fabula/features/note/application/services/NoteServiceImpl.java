...
public class NoteServiceImpl implements INoteService {
    ...
    @Override
    public ServiceResponse<Note> create(Note note) {
        try {
    ...
    @Override
    public ServiceResponse<Note> update(UUID id, Note entity) {
        try {
    ...
    public ServiceResponse<Boolean> delete(UUID id) {
        try {
            if (noteRepository.findById(id).isEmpty()){
                return new ServiceResponse<>("404","Note introuvable", false);
            }
